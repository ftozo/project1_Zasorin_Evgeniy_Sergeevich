...
ROOMS = {
    ...
    'trap_room': {
        ...
        'exits': {'west': 'entrance'},
        'items': ['rusty_key_from_treasure_room'],
    ...
    'secret_passage': {
        'description': 'Длинный темный узкий проход. В воздухе витает запах плесени. В конце прохода виднеется дверь, для нее нужен ключ.',
        'exits': {'south': 'armory', 'north': 'secret_room'},
        'items': [],
        'puzzle': None
    },
    'secret_room': {
        'description': 'Большая секретная комната. Посередине лежит старая ветхая карта, на которой указан путь к сокровищам.',
        'exits': {'south': 'secret_passage'},
        'items': ['old_map'],
        'puzzle': ('На стене нацарапано: "Три раза по два, минус один. Что получится?"', '5')
    }
}